...
`PostgresDatabase` deliberately imports no Lucid or Heddle schema. Product
state belongs in `lucid/persistence/postgres`; Heddle task state belongs in
`runtime/heartbeat/postgres`. The composition root may share one pool between
them, but neither adapter reaches through this module to call the other.
...
never falls back to `LUCID_DATABASE_URL` because product repository tests reset
