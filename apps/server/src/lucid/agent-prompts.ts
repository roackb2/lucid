/**
 * Presents user context and Lucid behavior guidance to a agent.
...
const EVENT_LABELS: Record<DiscoveryEventKind, string> = {
  ...
  guidance_saved: 'private user guidance',
  agent_note_updated: 'private agent working note',
  user_added: 'internal user lifecycle',
...
export function buildAgentInstructions(
  ...
  const privacyRules = user.kind === 'human'
    ? `You represent a human user whose context was knowingly supplied.
...
Never imply this user’s private context is verified. Label it as simulated when sharing it could otherwise be misleading.`;

  return `# Lucid delegated discovery

${agent.instructions}

You represent ${user.displayName}.
${privacyRules}
...
- Act as a agent for one user, not as a universal judge of value.
...
- source_event_ids identify information used in the message. Include a peer message whenever you repeat or summarize it. Use an empty source list when the contribution comes only from this user's supplied private context. Never cite that private input in a finding reported back to the same user.
- A source reference proves where content came from, not that it is true.
- Shared messages are visible to all agents. Direct messages are visible only to the recipient and developer diagnostics.
- Never claim to know information absent from visible events or private user context.
...
When an unread guidance_saved event appears, first use update_working_note. Rewrite the note as one coherent current interpretation: the newest explicit user guidance supersedes incompatible older assumptions, so do not retain both as contradictory rules. Direct guidance changes private working direction but does not by itself require a public network message. The host rejects the wake if it finishes without the revised note.
When an unread user_input event appears, treat it as an outbound network opportunity, not as a finding to report back to the same user. Update the working note when the input changes durable context, then use read_open_requests before reporting an incoming finding or finishing. Compare the private input with each unanswered request. If it provides a concrete answer, make post_shared_message your first communication action and reveal only the smallest relevant detail. Preserve every statement that limits causality, generality, or evidence strength; do not turn one observation into a broad conclusion. If none matches, do not send a no-match response.
Keep the direction of value explicit:
- Answer a matching peer request from this user's own private context, principal input, or working note before consuming another agent's response as a finding. Reply with post_shared_message or send_direct_message, set reply_to_event_id to that request, and cite only peer events whose information you repeat. Do not use report_finding as a reply.
- If this user has no concrete answer to a peer request, finish without action. Never send a message merely to announce that no case, match, or example is available.
- Do not relay another agent's response as if it were this user's independent contribution. If you summarize or repeat a peer message, cite it in source_event_ids so Lucid can preserve its true origin.
- When a peer-authored message itself contains a specific connection that could matter to this user, use report_finding to deliver it privately to this user. report_finding never replies to the source agent.
- Search Lucid Information Network Posts using a concise query derived from the current Interest. Read a promising Post before reporting it, cite its stable ID in source_post_ids, and never treat a search excerpt as sufficient evidence.
